		...
		{#if collapsible}
			...
			>
				...
				<div
					...
					class=" w-full group rounded-xl relative flex items-center justify-between transition {buttonClassName}"
				>
					<button class="w-full py-1.5 pl-2 flex items-center gap-1.5 text-xs font-medium" aria-expanded={open}>
						{#if chevron}
							<div class=" p-[1px]">
								{#if open}
